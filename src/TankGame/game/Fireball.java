...
public class Fireball {
    ...
    private static final int IMPACT_FRAME_DURATION = 75;
    private boolean hasImpacted = false;
    private boolean splashDamageApplied = false;

    public Fireball(float x, float y, float angle, BufferedImage image, List<BufferedImage> explosionFrames) {
        this.x = x;
        this.y = y;
        this.angle = angle;
        this.img = image;
        this.impactFrames = explosionFrames;
    ...
    public void setImageScale(float widthScale, float heightScale) {
        if (this.img != null) {
            this.img = resizeImage(this.img, (int) (this.img.getWidth() * widthScale), (int) (this.img.getHeight() * heightScale));
        }
    }
    ...
    public void deactivate(String reason, List<BreakableWall> breakableWalls, Tank otherTank, List<Drone> drones, List<Mine> mines) {
        ...
        Audio.playSound("tankgame-res/fireexplosion.wav", + 6.0f);
    ...
    private void applySplashDamage(List<BreakableWall> breakableWalls, Tank otherTank, List<Drone> drones, List<Mine> mines) {
        ...
        BufferedImage fifthFrame = impactFrames.get(3);
    ...
    public void draw(Graphics2D g2) {
        if (active) {
            AffineTransform rotation = AffineTransform.getTranslateInstance(x - img.getWidth() / 2.0, y - img.getHeight() / 2.0);
            ...
            int drawX = (int) (x - impactFrame.getWidth() / 2.0);
            int drawY = (int) (y - impactFrame.getHeight() / 2.0);
